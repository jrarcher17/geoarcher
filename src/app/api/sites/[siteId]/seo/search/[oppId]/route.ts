...
/** Update a search opportunity's workflow status. */
export async function PATCH(
  ...
  const existing = await prisma.seoSearchOpportunity.findUnique({
    where: { id: oppId },
  });
  ...
  const updated = await prisma.seoSearchOpportunity.update({
    where: { id: oppId },
