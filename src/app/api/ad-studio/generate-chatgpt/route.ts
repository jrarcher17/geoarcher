...
/** ChatGPT-only regeneration for Change Angle and New Version. */
export async function POST(request: Request) {
  ...
  try {
    const result = await generateChatgptAssets(offeringId, {
      name,
      goal,
      objectiveNote:
    ...
    console.error("[ad-studio] chatgpt generation failed:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "ChatGPT generation failed." },
      { status: 502 }
    );
  }
}
