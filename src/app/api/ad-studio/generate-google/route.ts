...
/** Google-only regeneration for Change Angle and New Version. */
export async function POST(request: Request) {
  ...
  try {
    const result = await generateGoogleAssets(offeringId, {
      name,
      goal,
      objectiveNote:
    ...
    console.error("[ad-studio] google generation failed:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Google generation failed." },
      { status: 502 }
    );
  }
}
